...
// Create the route for "/todos/:todoId/edit"
...
function EditTodoPage() {
  ...
  useEffect(() => {
    if (todo) {
      ...
      setRepeatEnabled(todo.repeat?.enabled || false)
      setRepeatDays(todo.repeat?.days || [])
    }
  }, [todo])
  ...
  return (
    <div className="page-transition min-h-screen pb-24">
      ...
      <header className="sticky top-0 z-10 glass-card rounded-none border-x-0 border-t-0 p-4">
        <button
          onClick={() => navigate({ to: '/todos/$todoId', params: { todoId } })}
          className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors mb-3"
        >
          <ArrowLeft className="w-5 h-5" />
          Back
        </button>
        <h1 className="text-2xl font-bold gradient-text">Edit Task</h1>
        <p className="text-muted-foreground text-sm mt-1">Update your task details</p>
      </header>
